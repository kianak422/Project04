...
saveLop.addEventListener('click', async()=>{
  const lop = {
...
saveSV.addEventListener('click', async()=>{
  const sv = {
  ...
  if(!res.ok){ alert('Lưu Sinh Viên thất bại: ' + (await res.text())); return; }
